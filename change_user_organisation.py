#in this script we move a super to a different organisation unit
...
from apiclient import discovery # from google
...
APPLICATION_NAME = 'G Suite: Change the users organization'
...
def sanitize_arguments():
    if len(sys.argv)<2:
        print('Error: Organisation name not recognized. ')
        sys.exit()
    name = ""
...
def main():
    ...
    parser.add_argument('-o', '--organization', help="""The name of the organization you want to add the user to.
    ...
    response = service.users().update(userKey=args.user,
        body={
            "orgUnitPath": "/"+args.organization,
        })
    try:
        response.execute()
    except HttpError:
        print("Error! The user or the organization you provided don't exist")
